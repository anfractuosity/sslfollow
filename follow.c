...
/*
    ...
    Extracts AES etc. keys from TLS connections via LD_PRELOAD
...
SECStatus PK11_Encrypt(PK11SymKey * symKey,
    ...
    SECStatus ret =
        next_encrypt(symKey, mechanism, param, out, outLen, maxLen, data,
                     dataLen);

    static CK_RV(*seckeycrypt) (CK_FUNCTION_LIST_PTR pFunctionList,
                                CK_SESSION_HANDLE hSession,
                                CK_OBJECT_HANDLE hSymKey,
                                CK_MECHANISM * cryptMech, const CK_BYTE * pData,
                                CK_ULONG dataLen) = NULL;
    ...
    CK_RV PKM_SecKeyCrypt(CK_FUNCTION_LIST_PTR pFunctionList,
        ...
        return seckeycrypt(pFunctionList,
                           hSession, hSymKey, cryptMech, pData, dataLen);
...
CK_SESSION_HANDLE GetNewSession(PK11SlotInfo * slot, PRBool * owner) {
...
SECStatus PK11_Decrypt(PK11SymKey * symkey,
    ...
    SECStatus ret =
        next_decrypt(symkey, mechanism, param, out, outLen, maxLen, enc,
                     encLen);
    return ret;
}
